...
window.onload = function init(){
 ...
 //event listeners for buttons
	...
	document.getElementById( "obj_drop" ).onclick = function () {
         mode =4;
    };
...
function render() {
    ...
    //
    lookAtMatrix = lookAt( vec3(0, .1, 0), vec3(0, 0, -10), vec3(0, 1, 0));	
	//lookAtMatrix = lookAt( vec3(0, 5, 0), vec3(0, 0, -10), vec3(0, 1, 0));	
    gl.viewport(0, -200, gl.viewportWidth, gl.viewportHeight);	
...
function drawScene() {
    pMatrix = perspective(120, gl.viewportWidth / gl.viewportHeight, 0.1, 100.0);
	
    // cube
    mvMatrix = translate(0.0, 1.0, -10.0);
    ...
    // sphere
    mvMatrix = translate(0.0, 15.0, -10.0);
    mvMatrix = mult(mvMatrix, rotate(rTotal, [0, 1, 0]));
    mvMatrix = mult(mvMatrix, translate(0.0, s_move, 0.0));
...
function setNextTimeScene() {
	...
	case 4:
		if(lastTime!=0){
			if (down_mode % 2 == 0) {
				s_move -= 0.01 + (1/2) * (9.8) * (timeNow - lastTime) * 0.00005;
				if (s_move < -18.0) {
					down_mode++;
					s_initial /= 2;
				}
